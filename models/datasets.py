...
class VGG_FACE_ADVERSARIAL(Dataset):
    ...
    def __init__(self, origin_path, adversarial_path):
        super().__init__()
        adversarial_data, self.l_enc = read_data(adversarial_path)
        orig_data, _ = read_data(origin_path)
...
class VGG_FACE_ADVERSARIAL_AUTOENCODER(Dataset):
    ...
    def __init__(self, origin_path, adversarial_path):
        adversarial_data, self.l_enc = read_data(adversarial_path)
        origin_data, _ = read_data(origin_path)
        origin_data = origin_data[origin_data.label.isin(adversarial_data.label.unique())]
